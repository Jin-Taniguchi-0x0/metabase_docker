...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    INSERT INTO wine_review (id, country, description, designation, points, price, province, region_1, region_2, taster_name, taster_twitter_handle, title, variety, winery, points_per_price)
    SELECT csv_id, country, description, designation, points, price, province, region_1, region_2, taster_name, taster_twitter_handle, title, variety, winery, ROUND(points::numeric / NULLIF(price::numeric, 0), 2) FROM wine_review_temp;
EOSQL
